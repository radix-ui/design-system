---
title: Avatar
component: Avatar
description: Useful for presenting profile images and initials.
---

```js live
<Avatar alt="Modulz" src="https://avatars0.githubusercontent.com/u/28682402">
  M
</Avatar>
```

<PropsTable
  data={{
    children: {
      type: 'ReactNode',
      description: 'The content to render',
    },
    alt: {
      type: 'string',
      description: 'A text description of the image',
    },
    src: {
      type: 'string',
      description: 'The URL of the image to use',
    },
    size: {
      type: '0 | 1',
      default: '0',
      description: 'The size to apply',
    },
  }}
/>

## Examples

### Size

There are 2 sizes to choose from.

```js live
<Avatar size={0} alt="Modulz" src="https://avatars0.githubusercontent.com/u/28682402">
  M
</Avatar>
<Avatar size={1} alt="Modulz" src="https://avatars0.githubusercontent.com/u/28682402" ml={4}>
  M
</Avatar>
```
